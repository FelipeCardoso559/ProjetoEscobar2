...
<h1>Meu Carrinho de Compras</h1>
            ...
            @foreach($carrinho as $produtoId => $item)
                <li>
                    ...
                    <form action="{{ route('carrinho.update', $item['produto']) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <button type="submit" name="change_quantidade" value="decrease" class="btn btn-primary btn-sm">-</button>
                        {{ $item['quantidade'] }}
                        <button type="submit" name="change_quantidade" value="increase" class="btn btn-primary btn-sm">+</button>
                    </form>
    ...
    @endif
        ...
        <a href="{{ route('produto.show') }}">
            <button class="btn btn-primary">Voltar para Produtos</button>
        </a>
